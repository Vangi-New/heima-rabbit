...
// 获取购物车数据
const cartList = ref<(CartItem & { show: boolean })[]>([])
// 优化购物车空列表状态，默认展示列表
...
const openSwipeAction = (index: number) => {
  // 先将正在被操作的swipeAction标记为打开状态，否则由于props的特性限制，
  // 原本为'false'，再次设置为'false'会无效
  cartList.value[index].show = true
  cartList.value.map((val, idx) => {
    if (index != idx) cartList.value[idx].show = false
  })
}
const onClickSwipeAction = (index: number, index1: number) => {
  if (index1 == 0) {
    // 删除
    let skuId: string = cartList.value[index].skuId
    // 弹窗二次确认
    uni.showModal({
      content: '是否删除',
      confirmColor: '#27BA9B',
